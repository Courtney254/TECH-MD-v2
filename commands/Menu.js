...
const fetchRepoStats = async () => {
    try {
        const response = await axios.get('https://api.github.com/repos/Courtney286/TECH-Md-V2');
...
module.exports = [
  ...
  return franceking();
        ...
        execute: async (king, msg, args, fromJid, allCommands) => {
            const time = moment().tz(config.timezone || 'Africa/Kenya');
            ...
            let text = `╭━━━❒ ${applyStyle("TECH-MD System INFO", 10)} ❒━━━╮\n`;
            text += `┃ 🐧 *Commands:* ${allCommands.length.toLocaleString()}\n`;
            text += `┃ 🦨 *Prefix:* ${prefix}\n`;
            text += `┃ 🕳️ *Time:* ${time.format('HH:mm:ss')}\n`;
  ...
  return franceking();
        ...
        execute: async (sock, msg, args, fromJid, allCommands) => {
            ...
            let text = `*🛠️ TECH-MD-V2 USER GUIDE*\n\n`;
            text += `To use the bot:\n`;
