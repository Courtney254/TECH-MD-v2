...
const isDev = ['254102174642', '254708459621'];
    ...
    execute: async (king, msg, args, fromJid) => {
      ...
      try {
        ...
        if (quoted.imageMessage) {
          ...
          sendMsg = {
            image: buffer,
            caption: quoted.imageMessage.caption || '*REVEALED BY TECH-MD*'
          };
        } else if (quoted.videoMessage) {
          const buffer = await getBuffer(quoted.videoMessage, 'video');
          sendMsg = {
            video: buffer,
            caption: quoted.videoMessage.caption || '*REVEALED BY TECH-MD*'
          };
        } else if (quoted.audioMessage) {
